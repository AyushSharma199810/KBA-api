...
app.post('/createLand', async (req, res) => {
  try {
    ...
    const createLandResponse = await contract.submitTransaction('CreateLand', landID, ownerName);
...
app.get('/readLand/:landID', async (req, res) => {
  try {
    ...
    const readLandResponse = await contract.evaluateTransaction('ReadLand', landID);
...
app.post('/enrollUser', async (req, res) => {
    try {
      ...
      res.status(200).send('User enrolled and identity added to wallet successfully.');
    } catch (error) {
